...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('command',choices=['integration','live','backfill']);p.add_argument('--output',type=Path,default=Path('chhss-data'));p.add_argument('--cache',type=Path,default=Path('.chhss-cache'));p.add_argument('--start',default='2024-01-01');p.add_argument('--end',default='2024-02-01');p.add_argument('--max-days',type=int);a=p.parse_args();a.output.mkdir(parents=True,exist_ok=True);acq=Acquire(a.cache)
    if a.command=='live':sys.exit(0 if live(acq,a.output) else 2)
    if a.command=='backfill':backfill(acq,date(a.start),date(a.end),a.output,a.max_days);return
    report=backfill(acq,date('2024-01-01'),date('2024-01-04'),a.output);ok=live(acq,a.output)
    try:
        r=acq.http.get(JSOC+'/jsocwiki/Lev1qualBits',timeout=45);r.raise_for_status();(a.output/'quality-bits-source.html').write_text(r.text)
    except Exception as e:print('Quality documentation retrieval:',e)
