/* Generated automatically by the SpaceWxOps WXF workflow. */
window.FLARE_GUIDANCE_PAYLOAD = {
  "schema_version": "4.1",
  "model_version": "sharp-mag-20260822-hierx1",
  "script_version": "1.3.0",
  "operational": false,
  "probability_scale": "percent",
  "issued": "2026-08-21T21:00:00Z",
  "valid_start": "2026-08-22T00:00:00Z",
  "valid_end": "2026-08-23T00:00:00Z",
  "quality": {
    "level": "research",
    "message": "Daily WXF inference from a saved calibrated M1+ model and a hierarchical X1+ severity layer. Research/shadow guidance unless explicitly validated and marked operational."
  },
  "input": {
    "series": "hmi.sharp_cea_720s_nrt",
    "target_time": "2026-08-21T18:00:00Z",
    "latest_record": "2026-08-21T12:36:00Z",
    "oldest_retained_record": "2026-08-21T12:36:00Z",
    "raw_records": 3472,
    "retained_regions": 1,
    "mapping": {
      "skipped_multi_region_harps": 1,
      "skipped_unmapped_harps": 20,
      "expanded_rows": 5
    },
    "quality": {
      "input": 5,
      "rejected_longitude": 4,
      "after_longitude": 1,
      "quality_missing": 0,
      "quality_top_values": {
        "0x00011C00": 1
      },
      "rejected_quality": 0,
      "after_quality": 1,
      "rejected_harp_merge": 0,
      "after_harp_merge": 1,
      "rejected_observer_velocity": 0,
      "after_observer_velocity": 1,
      "rejected_missing_mapping": 0,
      "after_mapping": 1,
      "min_finite_parameters": 12,
      "rejected_parameter_completeness": 0,
      "after_parameter_completeness": 1,
      "retained": 1
    }
  },
  "regions": [
    {
      "id": "full-disk",
      "label": "Full Disk",
      "quality": {
        "level": "research",
        "message": "Visible-disk WXF combination of quality-controlled, single-region HARPs. It contains no residual term for unnumbered, farside, or excluded limb regions."
      },
      "drivers": [
        "1 active regions represented",
        "Regional probabilities combined as 1 - product(1 - p_i)",
        "No unnumbered/farside residual term"
      ],
      "members": {
        "sharpmag": {
          "m1": 0.9,
          "x1": 0.1,
          "source": "WXF dominant-region proxy (maximum of 1 quality-controlled regional forecasts)",
          "quality": "research-dominant-region-proxy"
        },
        "mcstat": {
          "m1": 17.0,
          "x1": 3.0,
          "source": "SolarMonitor MCSTAT dominant-region proxy (maximum of 4 regional forecasts)",
          "quality": "published-comparison"
        },
        "mcevol": {
          "m1": 18.0,
          "x1": 0.0,
          "source": "SolarMonitor MCEVOL dominant-region proxy (maximum of 4 regions; MCSTAT fallback for 1 region(s) with no evolution forecast)",
          "quality": "published-comparison"
        },
        "sidc": {
          "m1": 0.0,
          "x1": 0.0,
          "source": "SIDC 24-hour Global Flare Forecast",
          "quality": "published-comparison",
          "issued": "2026-08-22T10:22:15Z",
          "valid_start": "2026-08-22T10:22:15Z",
          "valid_end": "2026-08-23T10:22:15Z",
          "note": "Human-operator-moderated global forecast published by SIDC."
        },
        "ccmc_magpy": {
          "m1": null,
          "x1": 14.0,
          "source": "NASA/CCMC Flare Scoreboard · CCMC MagPy",
          "quality": "published-comparison",
          "issued": "2026-08-21T14:03:32Z",
          "valid_start": "2026-08-21T11:00:00Z",
          "valid_end": "2026-08-22T11:00:00Z",
          "note": "Probability reproduced from the NASA/CCMC Flare Scoreboard HAPI feed.",
          "dataset_id": "MagPy_SHARP_HMI_CEA_FULLDISK"
        }
      }
    },
    {
      "id": "AR14511",
      "label": "AR 14511",
      "location": "",
      "mcintosh": "",
      "quality": {
        "level": "research",
        "message": "SHARP NRT record age 8.4 h; single-region HARP; |LON_FWT|=27.6°"
      },
      "members": {
        "sharpmag": {
          "m1": 0.9,
          "x1": 0.1,
          "source": "WXF sharp-mag-20260822-hierx1 (calibrated M1; hierarchical X1)",
          "quality": "research"
        }
      },
      "drivers": [
        "M1+: rising total unsigned flux",
        "M1+: lower mean magnetic shear",
        "M1+: elevated total unsigned vertical current",
        "X1+: hierarchical severity factor 6.1% of SHARP M1+"
      ]
    },
    {
      "id": "AR14508",
      "label": "AR 14508",
      "members": {
        "mcstat": {
          "m1": 0.0,
          "x1": 0.0,
          "source": "SolarMonitor MCSTAT regional forecast",
          "quality": "published-comparison"
        },
        "mcevol": {
          "m1": 0.0,
          "x1": 0.0,
          "source": "SolarMonitor MCEVOL regional forecast",
          "quality": "published-comparison"
        }
      }
    },
    {
      "id": "AR14513",
      "label": "AR 14513",
      "members": {
        "mcstat": {
          "m1": 17.0,
          "x1": 3.0,
          "source": "SolarMonitor MCSTAT regional forecast",
          "quality": "published-comparison"
        },
        "mcevol": {
          "m1": 18.0,
          "x1": 0.0,
          "source": "SolarMonitor MCEVOL regional forecast",
          "quality": "published-comparison"
        }
      }
    },
    {
      "id": "AR14514",
      "label": "AR 14514",
      "members": {
        "mcstat": {
          "m1": 1.0,
          "x1": 0.0,
          "source": "SolarMonitor MCSTAT regional forecast",
          "quality": "published-comparison"
        },
        "mcevol": {
          "m1": 0.0,
          "x1": 0.0,
          "source": "SolarMonitor MCEVOL regional forecast",
          "quality": "published-comparison"
        }
      }
    },
    {
      "id": "AR14515",
      "label": "AR 14515",
      "members": {
        "mcstat": {
          "m1": 0.0,
          "x1": 0.0,
          "source": "SolarMonitor MCSTAT regional forecast",
          "quality": "published-comparison"
        }
      }
    }
  ],
  "solar_monitor": {
    "source": "SolarMonitor",
    "source_url": "https://solmon.dias.ie/forecast?date=2026082200",
    "retrieved_at": "2026-08-22T20:41:54Z",
    "valid_start": "2026-08-22T00:00:00Z",
    "valid_end": "2026-08-23T00:00:00Z",
    "regional_forecasts": 4,
    "full_disk_method": "maximum regional probability (dominant-region proxy)",
    "mcevol_mcstat_fallback_regions_m1": 1,
    "mcevol_mcstat_fallback_regions_x1": 1,
    "note": "Regional MCSTAT/MCEVOL values are reproduced from SolarMonitor. SolarMonitor does not publish a full-disk aggregate in this table; the dashboard uses the maximum regional probability to avoid an independence-union inflation."
  },
  "wxf_full_disk": {
    "method": "maximum regional probability (dominant-region proxy)",
    "regional_forecasts": 1,
    "note": "Interim full-disk display proxy. It avoids an inflated independence-union calculation but is not a separately trained or calibrated full-disk model."
  },
  "external_sources": {
    "generated_at": "2026-08-22T20:41:54Z",
    "script_version": "1.0.0",
    "sidc_direct": {
      "ok": true,
      "url": "https://www.sidc.be/WMO/FlareForecast.php",
      "issued": "2026-08-22T10:22:15Z",
      "m1": 0.0,
      "x1": 0.0
    },
    "ccmc_catalog": {
      "ok": true,
      "url": "https://iswa.ccmc.gsfc.nasa.gov/IswaSystemWebApp/flarescoreboard/hapi/catalog",
      "datasets": 22
    },
    "ccmc_sidc": {
      "dataset_id": "SIDC_Operator_FULLDISK",
      "label": "SIDC",
      "ok": true,
      "url": "https://iswa.ccmc.gsfc.nasa.gov/IswaSystemWebApp/flarescoreboard/hapi/data?id=SIDC_Operator_FULLDISK&time.min=2026-08-18T21%3A00%3A00.0&time.max=2026-08-22T21%3A00%3A00.0&format=json&options=fields.all",
      "records": 3,
      "issued": "2026-08-21T12:30:03Z",
      "valid_start": "2026-08-21T12:30:00Z",
      "valid_end": "2026-08-22T12:30:00Z",
      "m1": null,
      "x1": 12.0
    },
    "ccmc_metoffice": {
      "dataset_id": "MO_TOT1_FULLDISK",
      "label": "Met Office",
      "ok": false,
      "url": "https://iswa.ccmc.gsfc.nasa.gov/IswaSystemWebApp/flarescoreboard/hapi/data?id=MO_TOT1_FULLDISK&time.min=2026-08-18T21%3A00%3A00.0&time.max=2026-08-22T21%3A00%3A00.0&format=json&options=fields.all",
      "records": 0,
      "detail": "No M/X probability record in query window"
    },
    "ccmc_ccmc_magpy": {
      "dataset_id": "MagPy_SHARP_HMI_CEA_FULLDISK",
      "label": "CCMC MagPy",
      "ok": true,
      "url": "https://iswa.ccmc.gsfc.nasa.gov/IswaSystemWebApp/flarescoreboard/hapi/data?id=MagPy_SHARP_HMI_CEA_FULLDISK&time.min=2026-08-18T21%3A00%3A00.0&time.max=2026-08-22T21%3A00%3A00.0&format=json&options=fields.all",
      "records": 56,
      "issued": "2026-08-21T14:03:32Z",
      "valid_start": "2026-08-21T11:00:00Z",
      "valid_end": "2026-08-22T11:00:00Z",
      "m1": null,
      "x1": 14.0
    },
    "ccmc_ccmc_daffs": {
      "dataset_id": "SPS_FULLDISK",
      "label": "CCMC DAFFS",
      "ok": false,
      "url": "https://iswa.ccmc.gsfc.nasa.gov/IswaSystemWebApp/flarescoreboard/hapi/data?id=SPS_FULLDISK&time.min=2026-08-18T21%3A00%3A00.0&time.max=2026-08-22T21%3A00%3A00.0&format=json&options=fields.all",
      "records": 2,
      "detail": "Forecast window does not meaningfully overlap target (2026-08-20T17:00:00Z to 2026-08-21T17:00:00Z)"
    },
    "ccmc_ccmc_aeffort": {
      "dataset_id": "SPS_FULLDISK",
      "label": "CCMC A-EFFort",
      "ok": false,
      "url": "https://iswa.ccmc.gsfc.nasa.gov/IswaSystemWebApp/flarescoreboard/hapi/data?id=SPS_FULLDISK&time.min=2026-08-18T21%3A00%3A00.0&time.max=2026-08-22T21%3A00%3A00.0&format=json&options=fields.all",
      "records": 2,
      "detail": "Forecast window does not meaningfully overlap target (2026-08-20T17:00:00Z to 2026-08-21T17:00:00Z)"
    },
    "flarecast": {
      "ok": false,
      "url": "https://api.flarecast.eu/api/prediction/flarecast_latest.xml",
      "http_status": 200,
      "content_length": 49,
      "detail": "XML contained no parseable current M/X probabilities",
      "candidate_count": 0
    }
  }
};
