(()=>{
  ...
  function chart(d,view='fluence',thresholdScale=true){
    ...
    if(thresholdScale&&!flux)max=Math.max(max,...(style?.thresholds('electron')||[{value:1.1e8}]).map(r=>r.value*1.12));
  ...
  function mount(root,flowRoot=root,initial=null){
    ...
    function paint(){
      ...
      if(d.status==='withheld'){root.innerHTML=modelControl()+`<p class="wx-particle-warning">Forecast withheld · ${esc(d.reason)}</p><p class="wx-particle-status">Issued ${utc(d.issuedAt)}. No missing data are substituted with zero.</p>`;wireModel();return;}
      ...
      root.innerHTML=modelControl()+`<div class="wx-particle-toolbar"><span class="wx-particle-status ${!active?'wx-particle-warning':''}">${active?(candidate?'SHADOW CANDIDATE':'EXPERIMENTAL'):'STALE SNAPSHOT'} · Data through ${utc(d.dataAsOf)} · Issued ${utc(d.issuedAt)}</span></div>
        ...
        <div class="wx-particle-toolbar"><label>Display <select data-wxf-view><option value="fluence">Rolling 24-hour fluence</option><option value="flux">Hourly electron flux</option></select></label><label><input data-wxf-scale type="checkbox" ${full?'checked':''}> Show threshold</label><span class="wx-particle-status">${esc(d.spacecraft||'GOES-19')} · >2 MeV · GEO</span></div>
        ${candidate?'<p class="wx-particle-warning">Research candidate: IPS/HSS phase and CME/HSS guidance are still undergoing verification. Current WXF remains the default.</p>':''}
        ...
        <p>In the candidate experiment, the larger dataset and measured drivers account for most of the improvement. Tests do not yet establish a reliable additional benefit from CME timing or recurrence alone. The September check and paired weekly-block comparisons are in the model card. Prospective verification remains pending.</p>
        <p>Office criterion: 1.1 × 10⁸ e⁻ cm⁻² sr⁻¹ in the preceding rolling 24 hours. ${e.thresholds.moderate.exceedanceCount} exceedances among ${e.thresholds.moderate.dailyOrigins} daily evaluation samples; adjacent days can belong to one event. No satellite-failure or threshold-exceedance probabilities are published. Editing Alert Settings changes plot styling, not historical validation counts.</p>
