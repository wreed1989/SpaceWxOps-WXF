...
function mount(root,parent){
 session??={draft:draftFor(),result:null,dirty:false,autoFollow:true};
 ...
 function eventOptions(){return '<option value="">Select a reported flare…</option>'+availableEvents().slice().reverse().map((event)=>`<option value="${esc(event.peakTime)}"${event.peakTime.slice(0,16)===session.draft.peakTime?' selected':''}>${esc(event.flareClass)} · ${utc(event.peakTime)}</option>`).join('');}
 function build(){
  root.innerHTML=`<section class="wx-sep-inputs"><h3>Model inputs</h3><div class="wx-particle-toolbar"><label>Load flare <select data-wxf-sep-event aria-label="Load observed flare">${eventOptions()}</select></label><button data-sep-apply>Run WXF Proton Model</button><button data-sep-background>Reload automatic inputs</button><button data-sep-reset>Clear inputs</button></div><p class="wx-sep-auto-status" data-sep-auto></p>
  ...
  root.querySelectorAll('[data-sep-input]').forEach(el=>el.oninput=()=>{
   ...
   if(key==='peakTime'){
    ...
    try{Object.assign(session.draft,automaticBackground(Date.parse(peakTime+'Z'),snapshot));}catch(e){session.draft.inputError=e.message;}
    build();return;
   }
   ...
   if(key==='background'&&el.value==='auto'){
    try{Object.assign(session.draft,automaticBackground(Date.parse(session.draft.peakTime+'Z'),snapshot));delete session.draft.inputError;build();return;}
    catch(e){session.draft.inputError=e.message;}
   }
   draftStatus();
  });
  root.querySelector('[data-wxf-sep-event]').onchange=e=>{if(e.target.value==='')return;session.draft=draftFor(availableEvents().find(event=>event.peakTime===e.target.value));session.autoFollow=false;session.dirty=true;build();};
  root.querySelector('[data-sep-reset]').onclick=()=>{session.draft=draftFor({});session.autoFollow=false;session.dirty=true;build();};
  root.querySelector('[data-sep-background]').onclick=()=>{
   const ev=availableEvents().find(event=>event.peakTime.slice(0,16)===session.draft.peakTime);
   if(!ev&&session.draft.peakTime){try{Object.assign(session.draft,automaticBackground(Date.parse(session.draft.peakTime+'Z'),snapshot),{background:'auto'});delete session.draft.inputError;session.dirty=true;build();}catch(e){root.querySelector('[data-sep-error]').textContent=e.message;}return;}
   session.draft=draftFor(ev);session.autoFollow=false;session.dirty=true;build();
  };
  root.querySelector('[data-sep-apply]').onclick=()=>{try{const result=run(session.draft);session.result=result;session.dirty=false;session.autoFollow=false;root.querySelector('[data-sep-error]').textContent='';paintResult();draftStatus();}catch(e){root.querySelector('[data-sep-error]').textContent=e.message;}};
  paintResult();paintObserved();draftStatus();
 }
 function draftStatus(){
  const d=session.draft,missing=number(d.longitude)===null||number(d.latitude)===null;
  ...
  root.querySelector('[data-sep-sources]').innerHTML=`<p class="wx-particle-status">Input snapshot ${utc(snapshot?.generatedAt)}. No model runs automatically. Peak edits clear the old event association; edits to pre-flare measurements switch to analyst mode.</p>${Object.entries(d.inputSources).filter(([,url])=>/^https:\/\//.test(url)).map(([key,url])=>`<p class="wx-particle-status"><a href="${esc(url)}" target="_blank" rel="noopener">${esc(key)} source</a></p>`).join('')}<p class="wx-particle-status">${(d.inputNotes||[]).map(esc).join(' ')}</p><p class="wx-particle-status">${(snapshot?.inputWarnings||[]).map(w=>esc(w.reason)).join(' · ')}</p>`;
 ...
 function paintObserved(){
  ...
  root.querySelector('[data-sep-feed]').textContent=`${error?error+' · ':''}GOES observations through ${utc(snapshot?.observations?.at(-1)?.time)}${Date.now()-Date.parse(snapshot?.observations?.at(-1)?.time)>90*60000?' · STALE':''}. Plots use your Alert Settings. Refresh updates observations and untouched automatic inputs; it never runs the model or overwrites analyst edits.`;
 }
...
window.SpaceWxProtonExperiment={mount,predict,refresh,run,automaticBackground,legacyEstimates,draftFor,availableEvents};
})();
