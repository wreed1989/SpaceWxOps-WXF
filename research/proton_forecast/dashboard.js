...
function mount(root,parent){
 ...
 const displayValue=key=>typeof session.draft[key]==='number'&&Number.isFinite(session.draft[key])?Number(session.draft[key].toPrecision(5)):session.draft[key];
 const field=(key,label,type='number',extra='')=>`<label>${label}<input data-sep-input="${key}" aria-label="${label}" type="${type}" value="${esc(displayValue(key))}" ${extra}></label>`;
 ...
 function build(){
  root.innerHTML=`<section class="wx-sep-inputs"><div class="wx-sep-setup-head"><span class="wx-sep-kicker">Event Selection</span><span data-sep-draft></span></div>
   <div class="wx-sep-control-row ${selectedTest()?'wx-sep-test-selected':''}"><label>Event Source<select data-sep-case aria-label="Proton Test Case"><option value="live"${selectedTest()?'':' selected'}>Live · Reported Flares</option>${testCases.map(c=>`<option value="${esc(c.id)}"${session.testCase===c.id?' selected':''}>${esc(c.title)}</option>`).join('')}</select></label><label ${selectedTest()?'hidden':''}>Reported Flare<select data-wxf-sep-event aria-label="Load Observed Flare">${eventOptions()}</select></label><button data-sep-apply aria-label="Run WXF Proton Model"><span aria-hidden="true">▶</span> Run Model</button></div>
   <p data-sep-test-note class="wx-sep-test-note"${selectedTest()?'':' hidden'}></p>
   <div class="wx-sep-core-groups"><fieldset><legend>Flare Observations</legend><div class="wx-sep-core-grid"><div class="wx-sep-wide">${field('peakTime','Flare Peak (UTC)','datetime-local','step="60"')}</div>${field('flareClass','Peak X-Ray Class','text','placeholder="e.g. M5.0"')}${field('riseMinutes','Rise Time (Minutes)','number','min="0" max="1440" step="any"')}</div></fieldset><fieldset><legend>Source &amp; Energy</legend><div class="wx-sep-core-grid">${field('latitude','Latitude: N(+) S(-)','number','min="-90" max="90" step="any" placeholder="Unknown"')}${field('longitude','Longitude: W(+) E(-)','number','min="-90" max="90" step="any" placeholder="Unknown"')}<div class="wx-sep-wide">${field('integral','Integrated X-Ray Flux (J/m²)','number','min="0" step="any"')}</div></div></fieldset></div>
   <div class="wx-sep-adjustments-heading">Analyst Adjustments</div>
   <details data-sep-manual><summary>Particle &amp; Flare History</summary><div class="wx-sep-input-grid">${select('background','Input Method',[['auto','Automatic · Pre-Flare GOES'],['manual','Analyst Supplied']])}</div><p class="wx-particle-status">Enter pre-flare medians from five-minute GOES samples. Current status uses the three five-minute samples before the window. Recent history checks the preceding 24 hours for a sustained ≥10 pfu crossing. Recent activity is context, not an automatic exclusion. Elevated pre-flare levels and trends remain model inputs; the probability concerns a new or renewed crossing in the stated window.</p><div class="wx-sep-input-grid">${field('P10','10 MeV · Last Hour Median (pfu)')}${field('P50','50 MeV · Last Hour Median (pfu)')}${field('P10Earlier','10 MeV · Preceding 5-Hour Median (pfu)')}${field('P50Earlier','50 MeV · Preceding 5-Hour Median (pfu)')}${field('priorFlares','Prior 24 Hours · Global C1+ Flare Count')}${select('P10Active','10 MeV ≥10 pfu Already Active?',[['unknown','Unknown · Withhold Probability'],['no','No'],['yes','Yes']])}${select('P50Active','50 MeV ≥10 pfu Already Active?',[['unknown','Unknown · Withhold Probability'],['no','No'],['yes','Yes']])}${select('P10Recent','10 MeV · Preceding 24 Hours',[['unknown','Unknown'],['clear','No Sustained Crossing'],['recent','Sustained Crossing']])}${select('P50Recent','50 MeV · Preceding 24 Hours',[['unknown','Unknown'],['clear','No Sustained Crossing'],['recent','Sustained Crossing']])}</div></details>
   <details data-sep-optional><summary>Peak Estimate &amp; Event Record</summary><p class="wx-particle-status">The optional published PROTONS relations estimate >10 MeV peak flux and flare-peak → proton-peak delay, conditional on an SEP event. Integrate to the decay level (peak + pre-flare background) / 2, including background in the integral. These estimates require C2.4+; they do not change the WXF occurrence probabilities.</p><div class="wx-sep-input-grid">${select('previous','Previous Flare From This Region?',[['unknown','Unknown'],['no','No'],['yes','Yes']])}${field('previousIntegral','Previous Flare Integral (J/m²)','number','min="0" step="any"')}</div><p class="wx-particle-status">Event record only: optical class and radio sweep are saved with this run but are not fitted predictors in WXF-SEP-0.1.</p><div class="wx-sep-input-grid">${field('opticalClass','Optical Class · Record Only','text')}${select('radio','Radio Sweep · Record Only',[['unknown','Unknown'],['none','None Observed'],['II','Type II'],['IV','Type IV'],['II+IV','Type II & IV']])}</div></details>
   <details><summary>Sources &amp; Quality</summary><p class="wx-sep-auto-status" data-sep-auto></p><div data-sep-sources></div></details><div class="wx-sep-setup-footer"><span data-sep-context></span><div><button data-sep-background>Reload Inputs</button><button data-sep-reset>Clear Inputs</button></div></div><p data-sep-error role="status"></p></section>
 ...
 function draftStatus(){
  ...
  root.querySelector('[data-sep-context]').textContent='Pre-Flare Background · '+['P10','P50'].map(ch=>`${ch==='P10'?'10':'50'} MeV: ${Number.isFinite(number(d[ch]))?Number(d[ch]).toPrecision(3)+' pfu':'Unknown'}`).join(' · ');
  ...
  root.querySelector('[data-sep-draft]').textContent=waiting?'Waiting For Peak +10 Min':(d.background==='auto'&&d.inputError)?'Input Coverage Incomplete':!ready?'Select Or Enter a Flare':session.dirty?'Changes Pending':session.result?'Run Complete':'Ready To Run';
  root.querySelector('[data-sep-sources]').innerHTML=`<p class="wx-particle-status">Input Snapshot ${utc(activeData()?.generatedAt)}. No model runs automatically. Peak edits clear the old event association; edits to pre-flare measurements switch to analyst mode.</p>${Object.entries(d.inputSources).filter(([,url])=>/^https:\/\//.test(url)).map(([key,url])=>`<p class="wx-particle-status"><a href="${esc(url)}" target="_blank" rel="noopener">${esc(key)} source</a></p>`).join('')}<p class="wx-particle-status">${(d.inputNotes||[]).map(esc).join(' ')}</p><p class="wx-particle-status">${d.inputError?esc(d.inputError)+' · ':''}${(activeData()?.inputWarnings||[]).map(w=>esc(w.reason)).join(' · ')}</p>`;
  window.SpaceWxLabels?.refresh(root);
 ...
 function paintResult(){
  ...
  el.innerHTML=`<p class="wx-sep-run-note"><b>${tc?'HISTORICAL TEST · ':''}${esc(r.event.flareClass)} · Peak ${utc(r.event.peakTime)}</b><br>Run ${utc(r.createdAt)} · ${r.input.background==='manual'?'Analyst-Supplied History':'Automatic Pre-Flare History'}${expired&&!tc?' · Historical Reconstruction':''}<br>Target Window: ${utc(r.validStart)} – ${utc(r.validEnd)}${late&&!tc?'<br>Late reconstruction: these are whole-window probabilities, not remaining risk.':''}</p>
   ...
   ${tc?`<div class="wx-sep-test-outcomes"><strong>Recorded Outcome</strong><p>${esc(tc.description)}</p><div class="wx-particle-table"><table><thead><tr><th>Target</th><th>Pre-Flare Context</th><th>Following 24 Hours</th><th>Peak (pfu)</th></tr></thead><tbody>${targets.map(([key,label])=>{const ch=key==='p50_10'?'P50':'P10',ctx=tc.episodeContext[ch];return `<tr><td>${label}</td><td>${ctx.status==='recent'?'Recent Event':ctx.status==='clear'?'No Recent Crossing':'Unknown'}${ctx.lastAbove?'<br><small>Last ≥10 pfu '+utc(ctx.lastAbove)+'</small>':''}</td><td>${tc.outcomes[key].label===1?'Threshold Crossed':tc.outcomes[key].label===0?'No Crossing':'Not Scored'}</td><td>${Number(tc.outcomes[key].observed_peak_pfu).toPrecision(3)}</td></tr>`;}).join('')}</tbody></table></div><small>${r.input.overrides.length?'Recorded observations refer to the original case, not the edited scenario.':'Archived observations; these are not additional validation samples.'}</small></div>`:''}
   ...
   <details><summary>Inputs Used In This Run</summary><p class="wx-particle-status">Peak ${esc(r.input.flareClass)} at ${utc(r.event.peakTime)} · rise ${esc(r.input.riseMinutes)} min · longitude ${esc(number(r.input.longitude)??'unknown')}° · W(+) E(-) · latitude ${esc(number(r.input.latitude)??'unknown')}° · N(+) S(-). Global Prior C1+ Flares: ${r.background.priorFlares}.</p><p class="wx-particle-status">Pre-Flare Medians: 10 MeV ${r.background.P10.toPrecision(4)} pfu (preceding five hours ${r.background.P10Earlier.toPrecision(4)}); 50 MeV ${r.background.P50.toPrecision(4)} pfu (preceding five hours ${r.background.P50Earlier.toPrecision(4)}). Baseline Active: 10 MeV ${esc(r.background.P10Active)}; 50 MeV ${esc(r.background.P50Active)}.</p><p class="wx-particle-status">Half-power SXR integral ${esc(number(r.input.integral)??'unknown')} J/m² · previous same-region flare ${esc(r.input.previous)} · previous integral ${esc(number(r.input.previousIntegral)??'unknown')} J/m². Record only: optical ${esc(r.input.opticalClass||'unknown')}; radio ${esc(r.input.radio)}.</p></details>`;
  window.SpaceWxLabels?.refresh(root);
  window.SpaceWxProductFlow?.sync(parent);
 }
