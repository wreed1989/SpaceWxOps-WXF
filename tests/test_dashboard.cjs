...
const navCode=['modelAvailable','productAvailable','modelNavigation'].map(name=>desk.match(new RegExp('  function '+name+'\\([^]*?\\n  \\}'))[0]).join('\n');
const navContext=vm.createContext({window:{},PRODUCT_LOOKUP:new Map([['model.nairas',{}],['realtime.imf',{}]]),PIPELINES:{huxt:{name:'CME | Solar Wind'},swig:{name:'Coronal Hole / HSS Outlook'},flare:{name:'Solar Flare Probability'},sep:{name:'SEP / Proton'},nairas:{name:'Radiation Dose · 20 km'},charging:{name:'Electron Forecast'}},state:{selectedModel:'huxt'},esc:s=>s});
vm.runInContext(navCode,navContext);
let nav=navContext.modelNavigation();
assert.equal((nav.match(/data-fd-model=/g)||[]).length,5);
assert.doesNotMatch(nav,/data-fd-model="nairas"/);
assert.equal(navContext.productAvailable('model.nairas'),false);
navContext.window.SpaceWxRadiationForecast={available:()=>true};
assert.equal(navContext.modelAvailable('nairas'),false); // A claim without a renderer is not usable.
navContext.window.SpaceWxRadiationForecast.mount=()=>{};
navContext.state.selectedModel='nairas';nav=navContext.modelNavigation();
assert.equal((nav.match(/data-fd-model=/g)||[]).length,6);
assert.match(nav,/data-fd-model="nairas" aria-current="page"/);
assert.equal(navContext.productAvailable('model.nairas'),true);
navContext.window.SpaceWxRadiationForecast.available=()=>{throw Error('unavailable')};
assert.equal(navContext.productAvailable('model.nairas'),false);
